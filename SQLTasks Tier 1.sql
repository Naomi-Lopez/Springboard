...
SELECT 
    ...
    m2.memid AS recommended_by_id, 
    CONCAT (m2.surname, '', m2.firstname) AS name_recommended_by 
FROM Members m1
...
SELECT f.name AS facility_name, COUNT(b.bookid) AS number_of_bookings
FROM Bookings as b 
INNER JOIN Facilities AS f ON b.facid = f.facid 
WHERE b.memid != 0
GROUP BY f.name;
...
SELECT f.name AS facility_name, COUNT(b.bookid) AS bookings_count, MONTH(starttime) AS month 
FROM Bookings AS b 
INNER JOIN Facilities AS f ON b.facid = f.facid 
WHERE b.memid != 0
